...
class VideoChatWindow(QDialog):
    def __init__(self, parent, tcp, my_nick, server_host):
        ...
        # 左上角：麦克风已关闭（全员同步）
        self.local_mic_mute_label = QLabel("麦克风已关闭")
        self.local_mic_mute_label.setStyleSheet("""
            color:white; 
            ...
            border-radius:4px;
        """)
        self.local_mic_mute_label.hide()

        # 右上角：听筒已关闭（全员同步）
        self.local_speaker_mute_label = QLabel("听筒已关闭")
        self.local_speaker_mute_label.setStyleSheet("""
            color:white; 
            background-color:rgba(220,0,0,0.85); 
            padding:4px 10px;
            font-size:13px;
            border-radius:4px;
        """)
        self.local_speaker_mute_label.hide()

        local_grid.addWidget(self.local_video, 0, 0)
        local_grid.addWidget(self.local_mic_mute_label, 0, 0, Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignLeft)
        local_grid.addWidget(self.local_speaker_mute_label, 0, 0, Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignRight)
        ...
        # 麦克风按钮
        self.btn_mic = QPushButton("静音")
        self.btn_mic.setFixedWidth(80)
        ...
        """)
        self.btn_mic.clicked.connect(self._toggle_mic_mute)

        # 听筒按钮
        self.btn_speaker = QPushButton("关闭听筒")
        self.btn_speaker.setFixedWidth(90)
        ...
        """)
        self.btn_speaker.clicked.connect(self._toggle_speaker_mute)
        ...
        bottom_bar.addWidget(self.btn_speaker)
    ...
    def update_mute_status(self, nick, muted):
        ...
        if nick == self.my_nick:
            self.local_mic_mute_label.setVisible(muted)
            # 同步本地按钮状态
            if muted:
                self.btn_mic.setText("取消静音")
                self.btn_mic.setStyleSheet("""
                    QPushButton {
            ...
            return
        if nick in self.remote_widgets:
            self.remote_widgets[nick]["mic_mute_label"].setVisible(muted)

    def update_speaker_mute_status(self, nick, muted):
        """更新指定参与者的听筒静音标识"""
        if nick == self.my_nick:
            self.local_speaker_mute_label.setVisible(muted)
            # 同步本地播放控制和按钮状态
            self.av_stream.set_speaker_mute(muted)
            if muted:
                self.btn_speaker.setText("打开听筒")
                self.btn_speaker.setStyleSheet("""
                    QPushButton {
                        background-color:#d00;
                        color:white;
            ...
            return
        if nick in self.remote_widgets:
            self.remote_widgets[nick]["speaker_mute_label"].setVisible(muted)
    ...
    def _add_remote_widget(self, nick):
        ...
        # 左上角：麦克风已关闭
        mic_mute_label = QLabel("麦克风已关闭")
        mic_mute_label.setStyleSheet("""
            color:white;
            background-color:rgba(220,0,0,0.85);
            padding:2px 6px;
            font-size:11px;
        ...
        # 右上角：听筒已关闭
        speaker_mute_label = QLabel("听筒已关闭")
        speaker_mute_label.setStyleSheet("""
            color:white;
            background-color:rgba(220,0,0,0.85);
            padding:2px 6px;
            font-size:11px;
            border-radius:3px;
        """)
        speaker_mute_label.hide()

        video_grid.addWidget(video_label, 0, 0)
        video_grid.addWidget(mic_mute_label, 0, 0, Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignLeft)
        video_grid.addWidget(speaker_mute_label, 0, 0, Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignRight)
        ...
        self.remote_widgets[nick] = {
            "container": container,
            "video": video_label,
            "mic_mute_label": mic_mute_label,
            "speaker_mute_label": speaker_mute_label
        }
    ...
    def _toggle_speaker_mute(self):
        ...
        if not self.av_stream:
            return
        current_muted = self.av_stream.speaker_muted
        new_muted = not current_muted

        # 发送状态同步指令
        cmd = "speaker_mute" if new_muted else "speaker_unmute"
        self.tcp.send_json({"cmd": cmd}, MSG_TYPE_VIDEO_INVITE)
